...
Fallible methods return `Error<I2C::Error>`. Invalid, non-finite, or unrepresentable physical configuration values and accumulator reads outside continuous mode return `Error::InvalidConfiguration`; bus failures return `Error::I2c`; range-scaled operations return `Error::AdcRangeUnknown` after an ambiguous range or reset write. Thresholds are rounded to the nearest register value.
