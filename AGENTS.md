...
Key state held in `Ina228`: `calibration: Option<Calibration>` (containing `current_lsb` and `shunt_resistance_ohm`) and a concrete `adc_range`. Construction reads CONFIG and synchronizes `adc_range` with the live device. Calibration is absent after construction and successful reset. Current, power, energy, charge, power-limit, and SHUNT_CAL calculations depend on it. Measurement freshness is a caller responsibility after reset or configuration changes.

`set_adc_range()` precomputes the new SHUNT_CAL because the calibration constant differs by 4× between ranges, writes CONFIG, then rewrites SHUNT_CAL. Cached range and calibration state changes only after the complete mutation succeeds. An I2C write error cannot prove whether the device accepted it, so every method that writes uses a fail-stop contract: reset or reconstruct the driver before further scaled operations. Do not add register-specific recovery state.
