...
Key state held in `Ina228`: `calibration: Option<Calibration>` (containing `current_lsb` and `shunt_resistance_ohm`), `adc_range`, and `shunt_voltage_stale`. Construction reads CONFIG and synchronizes `adc_range` with the live device. Calibration is absent after construction, reset, an ambiguous range write, or a failed range-dependent SHUNT_CAL rewrite. Current, power, energy, charge, power-limit, and SHUNT_CAL calculations depend on it. Reset and range changes mark VSHUNT stale until a completed shunt conversion is acknowledged.

`set_adc_range()` precomputes the new SHUNT_CAL because the calibration constant differs by 4× between ranges, writes CONFIG, then rewrites SHUNT_CAL. Failures before the CONFIG range write preserve the cached range and calibration. The cached range and calibration are invalidated immediately before that write because an I2C error cannot prove whether the device accepted it. After a successful CONFIG write, a failed SHUNT_CAL rewrite preserves the new range and leaves calibration unavailable. Preserve these transitions for new range/calibration paths.
