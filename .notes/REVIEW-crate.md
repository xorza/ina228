...
- [ ] The field's `<< 12` position is hardcoded in `configure` (`src/lib.rs:186`) and
...
- [ ] `.expect("call calibrate() before …")` is repeated with four different messages at
      `src/lib.rs:293`, `302`, `316`, `433`. The precondition is a property of the driver's
...
- [ ] `Ina228` lives in `src/lib.rs` rather than `src/ina228.rs`, against "one major
      struct, one file, same name". Its satellites (`AdcConfig`, `AlertConfig`,
      `DiagnosticFlags`, `AccumulatorSnapshot`) and the two `encode_*` free fns belong
...
- [ ] Ten of `DiagnosticFlags`'s eleven public fields are undocumented
      ...
      `AccumulatorSnapshot`'s three are undocumented (`src/lib.rs:138-139`). All variants
...
- [ ] The `AlertConfig` example is a ```` ```ignore ```` doctest (`src/lib.rs:102`) —
